...
class GraphAttentionConv(nn.Module):
    def __init__(self, emb_dim, negative_slope=0.2, **kwargs):
        ...
        self.attn_user_l = nn.Parameter(torch.FloatTensor(size=(1, emb_dim)))
        self.attn_user_r = nn.Parameter(torch.FloatTensor(size=(1, emb_dim)))
        self.attn_item_l = nn.Parameter(torch.FloatTensor(size=(1, emb_dim)))
        self.attn_item_r = nn.Parameter(torch.FloatTensor(size=(1, emb_dim)))
        self.leaky_relu = nn.LeakyReLU(negative_slope)
        self.reset_parameters()

    def reset_parameters(self):
        gain = nn.init.calculate_gain('relu')
        nn.init.xavier_normal_(self.attn_user_l, gain=gain)
        nn.init.xavier_normal_(self.attn_user_r, gain=gain)
        nn.init.xavier_normal_(self.attn_item_l, gain=gain)
        nn.init.xavier_normal_(self.attn_item_r, gain=gain)
    ...
    def forward(self, G, feat_dict):
        ...
        user_el = (user_src_embs * self.attn_user_l).sum(dim=-1).unsqueeze(-1)
        user_er = (user_dst_embs * self.attn_user_r).sum(dim=-1).unsqueeze(-1)

        item_el = (item_src_embs * self.attn_item_l).sum(dim=-1).unsqueeze(-1)
        item_er = (item_dst_embs * self.attn_item_r).sum(dim=-1).unsqueeze(-1) 
...
#===============================Model Defination===============================
class LightGATType(nn.Module):
    def __init__(self, n_users, n_items, emb_dim, 
                 n_layers, batch_size, decay, *args, **kwargs):
        super(LightGATType, self).__init__()
