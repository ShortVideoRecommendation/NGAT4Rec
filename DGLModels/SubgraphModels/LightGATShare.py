...
class GraphAttentionConv(nn.Module):
    def __init__(self, emb_dim, negative_slope=0.2, **kwargs):
        ...
        self.attn_l = nn.Parameter(torch.FloatTensor(size=(1, emb_dim)))
        self.attn_r = nn.Parameter(torch.FloatTensor(size=(1, emb_dim)))
    ...
    def forward(self, G, feat_dict):
        ...
        user_el = (user_src_embs * self.attn_l).sum(dim=-1).unsqueeze(-1)
        user_er = (user_dst_embs * self.attn_r).sum(dim=-1).unsqueeze(-1)

        item_el = (item_src_embs * self.attn_l).sum(dim=-1).unsqueeze(-1)
        item_er = (item_dst_embs * self.attn_r).sum(dim=-1).unsqueeze(-1) 
...
#===============================Model Defination===============================
class LightGATShare(nn.Module):
    def __init__(self, n_users, n_items, emb_dim, 
                 n_layers, batch_size, decay, *args, **kwargs):
        super(LightGATShare, self).__init__()
